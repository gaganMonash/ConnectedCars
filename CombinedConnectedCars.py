...
def repeat(object):
    ...
    if object!= None:
        ...
        data=object+","
    print("-------------------------------")
    data=data+oilVal+","+timeVal+","+speedVal+","+fuelVal+","+rpmVal+","+accelerationPosDVal+","+coolantTempVal+","+engineLoadVal+"\n"
    writetoFile(data)        
...
#Kishore change- framerate from 30 to 60
videostream = VideoStream(resolution=(imW,imH),framerate=2000).start()
time.sleep(1)
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            print("image detected:",object_name )
            print("Calling Gagan's Method with object")
            repeat(object_name)
        else:
            print("Calling Gagan's Method withoutttttt object")
            repeat(None)
            #ime.sleep(1)
